...
import { Eye, MoreVertical, UserCheck, UserX, CheckCircle, XCircle, Clock } from "lucide-react";
...
export default function ModernTable({ data, type, onAction }: ModernTableProps) {
  ...
  const renderUserCard = (user: any) => (
    <Card key={user.id} className="bg-white shadow-md hover:shadow-lg transition-all duration-200 border border-gray-100">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-500 truncate">{user.telegramId}</p>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="text-center">
              <Badge className={`${getStatusColor(user.status)} px-3 py-1 font-medium`} variant="outline">
            ...
            <div className="flex gap-2">
              <Button 
                variant="outline" 
                size="sm" 
                className="h-9 px-3 rounded-lg border-gray-200 hover:bg-gray-50"
  ...
  const renderPaymentCard = (payment: any) => (
    <Card key={payment.id} className="bg-white shadow-md hover:shadow-lg transition-all duration-200 border border-gray-100">
      <CardContent className="p-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-500 truncate">{payment.profiles?.email}</p>
              <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <Badge className={`${getStatusColor(payment.status)} px-3 py-1 font-medium`} variant="outline">
  ...
  const renderPayoutCard = (payout: any) => (
    <Card key={payout.id} className="bg-white shadow-md hover:shadow-lg transition-all duration-200 border border-gray-100">
      <CardContent className="p-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xl font-bold text-gray-900">₹{payout.amount}</p>
              <p className="text-sm text-gray-500 truncate">{payout.profiles?.email}</p>
              <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <Badge className={`${getStatusColor(payout.status)} px-3 py-1 font-medium`} variant="outline">
            ...
            {payout.status === "pending" && (
              <div className="flex gap-2">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="h-9 px-4 border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300 rounded-lg"
